...
export default function MentorReferenceLetterDetailPage({ params }: PageProps) {
  ...
  if (isLoading) {
    return (
      <PageLoader text="Memuat data berkas surat keterangan..." spinnerColor="text-indigo-500" />
    );
  }
  ...
  if (!student) {
    return (
      <NotFoundBlock
        ...
        description={`Data mahasiswa dengan ID #${unwrappedParams.id} tidak terdaftar di sistem penerbitan surat keterangan magang.`}
        backHref="/dashboard/mentor/surat-keterangan"
        backLabel="Kembali ke Daftar Surat Keterangan"
      />
    );
  }
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="glass-card border border-slate-200/50 dark:border-slate-800/80 rounded-3xl p-6 md:p-8 shadow-sm bg-white dark:bg-[#070e24]/40 space-y-6">
            ...
            <form onSubmit={handleSaveLetter} className="space-y-6">
              ...
              {/* Form Action buttons */}
              <ModalActions
                cancelHref="/dashboard/mentor/surat-keterangan"
